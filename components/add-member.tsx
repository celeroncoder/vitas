...
export const AddMemberForm: React.FC<{ project: Project }> = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (payload.success) {
      ...
      if (res) {
        toast({
          title: `Participant @${res.username} Added!`,
        ...
        toast({
          ...
          description:
            "Uh Oh! Some problem Occurred while adding the participant",
        });
    } else {
      toast({
        title: "Please Provide Correct Data",
        description:
          "Invalid Details for the participant provided, please change them. The form has been reset fill it with correct details.",
      });
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <PlusIcon className="w-4 mr-2" /> Add Participant
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add Participant</DialogTitle>
          <DialogDescription>
            Add a new participant to {project.name}
          </DialogDescription>
        </DialogHeader>
