...
export const AddMembersCSVForm: React.FC<{ project: Project }> = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    if (rows && cols) {
      try {
        ...
        if (res) {
          toast({
            title: `${res.count} Participants Added!`,
            description: `Participants were added from the csv with the specified column mapping successfully!`,
          });
        } else
          toast({
            title: "Some Error Occurred!",
            description:
              "Uh Oh! Some problem Occurred while adding the participants",
          });
      } catch (err) {
        toast({
          title: "Some Error Occurred!",
      ...
      toast({
        ...
        description:
          "Please map each memeber field with a column to add multiple participants.",
        variant: "destructive",
      });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <UploadIcon className="w-4 mr-2" /> Add Participants
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add Participants</DialogTitle>
          <DialogDescription>
            Upload a CSV file and map columns to the participant fields, and
            mass create participants in the event.
          </DialogDescription>
        </DialogHeader>
            ...
            <section className="mb-2 animate-in duratio-300">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Map the Column from the uploaded csv to participant fields.
                </p>
              </div>
